...
def get_atmosphere(I, brightch, p):
    ...
    searchidx = (-flatbright).argsort()[
        : int(M * N * p)
    ]  # find top M * N * p indexes. argsort() returns sorted (ascending) index.

    # return the mean intensity for each channel
    A = np.mean(
        flatI.take(searchidx, axis=0), dtype=np.float64, axis=0
    )  # 'take' get value from index.
...
def get_initial_transmission(A, brightch):
    ...
    init_t = (brightch - A_c) / (1.0 - A_c)  # original
...
def dehaze(I, tmin, w, alpha, omega, p, eps):
    ...
    print(
        "dark max:{} min:{}".format(np.max(Idark), np.min(Idark))
    )  # dark max:0.776470588235 min:0.0
    print(
        "bright max:{} min:{}".format(np.max(Ibright), np.min(Ibright))
    )  # bright max:0.988235294118 min:0.00392156862745
    ...
    print(
        "initial (bright) transmission rate between [%.4f, %.4f]"
    ...
    print(
        "refined transmission rate between [%.4f, %.4f]"
    ...
    print(
        "refined dark transmission rate between [%.4f, %.4f]"
...
if __name__ == "__main__":
    ...
    I = (
        I[:, :, :3] / 255
    )  # See here. https://stackoverflow.com/questions/44955656/how-to-convert-rgb-pil-image-to-numpy-array-with-3-channels

    with open(folder + "/info.txt", mode="w") as f:
        s = "tmin={}\nw={}\nalpha={}\nomega={}\np={}\neps={}".format(
            tmin, w, alpha, omega, p, eps
        )
        f.write(s)
